...
# include <ME/includes/start.incl>
  ...
  "T2b = T2*(1 - timeCACO/T2max)/2"
  "if (T2max <= timeCACO) {T2b = 0;}"
  "T2c = timeCACO*(1 - T2/T2max)/2"
  "if (T2max <= timeCACO) {T2c = (timeCACO - T2)/2;}"
...
; 2CAzCOz -> 2CAzNz with CO evolution (T2):
